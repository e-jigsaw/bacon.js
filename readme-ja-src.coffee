...
もし[node.js](http://nodejs.org/)で使うなら

    npm install baconjs

[bower](https://github.com/twitter/bower) を使うなら
...
さて Bacon.js を使えば以下のように「要素をクリックする」のようなイベントを`EventStream`にラップすることができる。

```js
var clicks = $("h1").asEventStream("click")
```

それぞれの`EventStream`はイベントの流れ(ストリーム)を表しており、流れてくるイベントは例えば[`onValue`](#stream-onvalue)メソッドとコールバック関数を使えば次のように観測することができる。
...
`scan`メソッドは与えられたシードの値から始まり入力のストリームからのそれぞれのイベントで蓄積関数に現在のPropertyの値とストリームからの新しい値を作用させて新しいPropertyを作っている。
